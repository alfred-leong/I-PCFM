...
def parse_args():
    ...
    parser.add_argument('--exp5_constraint_density', action='store_true',
        help='Run Exp 5: sweep k (dynamics unrolling steps) across methods')
    parser.add_argument('--k_values', type=str, default='1,5,10,20',
        help='Comma-separated k values for Exp 5 dynamics constraint density sweep')
    parser.add_argument('--exp1_equality_k', type=int, default=20,
        help='Exp 1: k (dynamics unrolling steps) for equality-only methods (vanilla, pcfm_equality). Default 20 matches the I-PCFM methods for a symmetric comparison.')
...
def compute_metrics(u_pred: torch.Tensor, u_true: torch.Tensor,
    ...
    inf = float('inf')
    device_t = u_pred.device
    ic_per = torch.full((B,), inf, device=device_t)
    cl_per = torch.full((B,), inf, device=device_t)
    max_ineq_per = torch.full((B,), inf, device=device_t)
    ineq_viol_norm_per = torch.full((B,), inf, device=device_t)
    ...
    for i in range(B):
        ...
        if hfuncs is not None:
            res_obj = Residuals(
                data=u_true[i:i+1], x=torch.linspace(0, 1, nx, device=device_t),
                t_grid=torch.linspace(0, 1, nt, device=device_t),
            ...
            cl_per[i] = res_obj.mass_residual_burgers(u_flat)[1:].norm()

    # Aggregate over valid samples
    mask = valid_mask
    ce_ineq = ineq_viol_norm_per[mask].mean().item() if mask.any() else float('nan')
    if hfuncs is not None:
        ce_ic = ic_per[mask].mean().item() if mask.any() else float('nan')
        ce_cl = cl_per[mask].mean().item() if mask.any() else float('nan')
    else:
        ce_ic = float('nan')
        ce_cl = float('nan')
    ...
    if hfuncs is not None:
        ...
        joint_feasibility_rate = float('nan')
...
def run_exp1_main(args, model, u_true, device):
    ...
    k_eq = args.exp1_equality_k
    k_ipcfm = 20
    hfuncs_eq = [build_hfunc(u_true[j], device, nx=101, nt=101, k=k_eq) for j in range(B)]
    hfuncs_ipcfm = [build_hfunc(u_true[j], device, nx=101, nt=101, k=k_ipcfm) for j in range(B)]
    ineq = build_entropy_ineq(device, nx=101, nt=101)

    for method_name in methods:
        print(f'\nRunning {method_name}...')
        hfuncs = hfuncs_eq if method_name in EQUALITY_METHODS else hfuncs_ipcfm
        try:
...
def _print_latex_table(results):
    header = r'\begin{tabular}{lrrrrrrrr}'
    cols = r'Method & CE(IC) & CE(CL) & CE-Ineq & Feas(ineq) & Feas(joint) & MMSE & SMSE & Time(s/sample) \\'
    print('\n' + header)
    print(r'\hline')
    print(cols)
    print(r'\hline')
...
def _plot_exp2_tradeoff(results, out_path):
    ...
    colors = {'ipcfm_a': 'blue', 'ipcfm_b': 'red', 'ipcfm_c': 'green',
              'pcfm_equality': 'gray', 'vanilla': 'black'}
...
def _plot_exp4_active_set(tau_list, means, stds, out_path):
    ...
    ax.set_title('Active-Set Size vs Flow Time')
    ax.legend()
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(out_path, dpi=150)
...
def run_exp5_constraint_density(args, model, u_true, device):
    """Sweep equality constraint density k for pcfm_equality and I-PCFM A/B/C."""
    print('\n' + '='*60)
    print('Experiment 5: Equality Constraint Density vs Inequality Satisfaction')
    print('='*60)

    k_values = [int(x) for x in args.k_values.split(',')]
    eligible_methods = ['pcfm_equality', 'ipcfm_a', 'ipcfm_b', 'ipcfm_c']

    if args.method == 'all':
        methods = eligible_methods
    elif args.method in eligible_methods:
        methods = [args.method]
    else:
        print(f'[WARNING] Method {args.method} not eligible for Exp 5. Using all.')
        methods = eligible_methods
    ...
    for method_name in methods:
        ...
        for k_val in k_values:
            print(f'\n  {method_name}, k={k_val}...')
            hfuncs = [build_hfunc(u_true[j], device, nx=101, nt=101, k=k_val) for j in range(B)]
            try:
                method_kwargs = _build_method_kwargs(args, method_name)
                u_pred, tps = run_sampling(
                    model, u_true, method_name, args.n_steps,
                ...
                metrics['time_per_sample_s'] = tps
                metrics['k'] = k_val
                results[method_name][str(k_val)] = metrics
                print(f'    {metrics}')
            except Exception as e:
                print(f'    ERROR: {e}')
                save_error_log(args.results_dir, f'exp5_{method_name}_k{k_val}', str(e))
                results[method_name][str(k_val)] = {'error': str(e), 'k': k_val}

    out_path = os.path.join(args.results_dir, f'exp5_constraint_density{args.result_suffix}.json')
    if os.path.exists(out_path):
        with open(out_path) as f:
            existing = json.load(f)
        for m, kdict in results.items():
            if m in existing:
                existing[m].update(kdict)
            else:
                existing[m] = kdict
        results = existing
    save_json(results, out_path)

    _plot_exp5_constraint_density(
        results, k_values,
        os.path.join(args.results_dir, f'exp5_constraint_density{args.result_suffix}.png'))

    summary = f'\n## Exp 5: Equality Constraint Density\n'
    summary += f'- k values: {k_values}\n'
    for m, kdict in results.items():
        for kk, v in kdict.items():
            summary += f'- {m} k={kk}: {v}\n'
    append_summary(args.results_dir, summary)
...
def _plot_exp5_constraint_density(results, k_values, out_path):
    """Plot key metrics vs k for each method -- 2x2 grid."""
    fig, axes = plt.subplots(2, 2, figsize=(12, 10))
    metric_keys = ['ce_ineq', 'feasibility_rate', 'ce_cl', 'mmse']
    metric_labels = ['CE-Ineq (lower better)', 'Feasibility Rate (higher better)',
                     'CE(CL) (lower better)', 'MMSE (lower better)']
    colors = {
        'pcfm_equality': 'gray',
        'ipcfm_a': 'blue',
        'ipcfm_b': 'red',
    ...
    for ax, metric_key, label in zip(axes.flat, metric_keys, metric_labels):
        for method_name, kdict in results.items():
            ks_valid = []
            vals = []
            for k_str in sorted(kdict.keys(), key=lambda x: int(x)):
                m = kdict[k_str]
                if 'error' not in m and metric_key in m:
                    ks_valid.append(int(k_str))
                    vals.append(m[metric_key])
            if ks_valid:
                ax.plot(ks_valid, vals,
                        color=colors.get(method_name, 'purple'),
                        marker=markers.get(method_name, 'x'),
                        linewidth=2, markersize=8, label=method_name)
        ax.set_xlabel('k (dynamics unrolling steps)')
        ax.set_ylabel(label)
        ax.set_title(label)
    ...
    fig.suptitle('Exp 5: Equality Constraint Density vs Metrics', fontsize=14)
    plt.tight_layout()
    plt.savefig(out_path, dpi=150)
    plt.close()
    print(f'Saved plot: {out_path}')
...
def main():
    ...
    # except when the user has explicitly requested another experiment
    if args.exp2_sweep or (args.method == 'ipcfm_a' and not (
            args.exp1_main or args.exp3_timing or args.exp4_active_set or args.exp5_constraint_density)):
        any_exp_run = True
        try:
    ...
    if args.exp5_constraint_density:
        any_exp_run = True
        try:
            results = run_exp5_constraint_density(args, model, u_true, device)
            wandb_log(wb, {'exp5_complete': 1})
        except Exception as e:
            save_error_log(args.results_dir, 'exp5', str(e))
            print(f'[ERROR] Exp 5 failed: {e}')
